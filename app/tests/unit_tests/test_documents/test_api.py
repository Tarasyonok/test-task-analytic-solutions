...
@pytest.mark.parametrize("search_text,result,status_code", [
  ("Зелёное яблоко", green_apple, 200),
  ("Зеленое, яблоко!", green_apple, 200),
  ("яблоко зелёное", green_apple, 200),
  ("привет", hello, 200),
  ("То, чего точно нет в тексте", [], 200),
  ("", [], 200),
])
async def test_search_documents(search_text, result, status_code, ac: AsyncClient):
    response = await ac.get("documents", params={"search_text": search_text})
    assert response.status_code == status_code
    assert response.json() == result
    
    
@pytest.mark.parametrize("document_id,status_code", [
  (4, 200),
  (10, 200),
])
async def test_delete_documents(document_id, status_code, ac: AsyncClient, session):
    response = await ac.delete(f"documents/{document_id}")
    assert response.status_code == status_code
